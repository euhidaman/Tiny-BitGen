...
def download_coco_dataset(data_dir: str = "./data") -> Dict:
    ...
    try:
        ...
        try:
            ...
            downloaded_path = kagglehub.download(
                "nikhil7280/coco-image-caption")
            logger.info(
                f"✅ Dataset downloaded with alternative API to: {downloaded_path}")
            ...
            if train_source.exists():
                logger.info(
                    "📂 Copying train2014 images to unified directory...")
                for img_file in train_source.glob("*.jpg"):
                    dest_path = all_images_dest / img_file.name
                    if not dest_path.exists():
                        shutil.copy2(img_file, dest_path)
...
def process_coco_captions(data_dir: str = "./data") -> List[Dict]:
    ...
    if trainval_captions_filepath.exists():
        ...
        for _, row in trainval_captions_df.iterrows():
            ...
            image_path = images_dir / \
                image_filename if images_dir.exists() else train_images_dir / image_filename
        ...
        logger.info(
            f"   Found {len([p for p in image_caption_pairs if p['dataset'] == 'coco_train2014']):,} valid train2014 image-caption pairs")
    ...
    if test_captions_filepath.exists():
        ...
        for _, row in test_captions_df.iterrows():
            ...
            # Try unified directory first
            image_path = images_dir / image_filename if images_dir.exists() else val_images_dir / \
                image_filename
        ...
        logger.info(
            f"   Found {len([p for p in image_caption_pairs if p['dataset'] == 'coco_val2017']):,} valid val2017 image-caption pairs")

    logger.info(
        f"✅ Total COCO image-caption pairs: {len(image_caption_pairs):,}")
...
def main():
    ...
    logger.info(
        f"   COCO: {status['coco_count']:,} images" if status['coco_exists'] else "   COCO: Not found")
    logger.info(
        f"   Features Cache: {status['existing_features_count']:,} features" if status['features_cache_exists'] else "   Features Cache: Not found")
    ...
    if save_result:
        logger.info("\n🎉 SUCCESS!")
        logger.info(
            f"   📊 Total dataset size: {save_result['total_samples']:,} samples")
        logger.info(
            f"   🖼️ COCO contribution: {save_result['coco_samples']:,}")
        logger.info(
            f"   ⚡ Feature extraction: {save_result['feature_extraction']}")
