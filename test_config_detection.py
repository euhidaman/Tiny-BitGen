...
def test_config_detection():
    ...
    try:
        ...
        print(
            f"   - Has token_constraints: {'token_constraints' in coco_config}")
        if 'token_constraints' in coco_config:
            print(
                f"   - Token constraints enabled: {coco_config['token_constraints'].get('enabled', False)}")

        print(
            f"   - Dataset: {coco_config.get('data', {}).get('dataset_name', 'Unknown')}")
    ...
    try:
        ...
        print(
            f"   - Has token_constraints: {'token_constraints' in babylm_config}")
        if 'token_constraints' in babylm_config:
            print(
                f"   - Token constraints enabled: {babylm_config['token_constraints'].get('enabled', False)}")
            print(
                f"   - Target tokens: {babylm_config['token_constraints'].get('total_tokens', 'Unknown'):,}")

        print(
            f"   - Dataset: {babylm_config.get('data', {}).get('dataset_name', 'Unknown')}")
